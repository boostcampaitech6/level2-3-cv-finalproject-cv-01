...
def get_CNN5d_5d():
    ...
    state_dict = torch.load('model/weight/I5R5_Model.tar',map_location=torch.device(device))
...
def get_CNN5d_20d():
    model = CNN5d()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    state_dict = torch.load('model/weight/I5R20_Model.tar',map_location=torch.device(device))
    model.load_state_dict(state_dict['model_state_dict'])
    model = model.to(device)
    return model
...
def get_CNN20d_5d():
    model = CNN20d()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    state_dict = torch.load('model/weight/I20R5_Model.tar',map_location=torch.device(device))
    model.load_state_dict(state_dict['model_state_dict'])
    model = model.to(device)
    return model
...
def get_CNN20d_20d():
    model = CNN20d()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    state_dict = torch.load('model/weight/I20R20_Model.tar',map_location=torch.device(device))
    model.load_state_dict(state_dict['model_state_dict'])
    model = model.to(device)
    return model