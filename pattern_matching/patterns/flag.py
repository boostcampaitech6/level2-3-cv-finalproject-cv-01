...
def fig_flag_points(company, fig, ohlc, flag_points, back_candles):
...
def plot_flag_pattern(data, company, base_fig, success_patterns):
    # 1. flag pattern
    ohlc_flag = get_ohlc(data)
    back_candles_flag = 20
    flag_points = find_flag_points(ohlc_flag, back_candles_flag)
    
    if flag_points:
        success_patterns.append("flag")
        flag_fig = go.Figure(base_fig)
        flag_fig = fig_flag_points(company, flag_fig, ohlc_flag, flag_points, back_candles_flag)   
