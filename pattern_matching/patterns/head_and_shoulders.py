...
def find_head_and_shoulders_points(df, candle_id, back_candles):
    """
    Find points provides all the necessary arrays and data of interest
...
def find_head_and_shoulders(df: pd.DataFrame, back_candles: int = 14) -> List[int]:
    ...
    for candle_id in range(back_candles+20, len(df)-back_candles):
        ...
        # if maxim[headidx]-maxim[headidx-1]>1.5e-3 and maxim[headidx]-maxim[headidx+1]>1.5e-3 and abs(slmin)<=1e-4: 
            # all_points.append(candle_id)
        if headidx > 0 and headidx < len(maxim) - 1:
...
def save_plot(ohlc, all_points, back_candles, save_dir, fname="head_and_shoulders", hs=True):
    """
    Save all the graphs
...
if __name__ == "__main__":
    ...
    # /data/ephemeral/home/Final_Project/level2-3-cv-finalproject-cv-01/stock_data/naver_data.csv
    ...
    save_dir = save_dir = '/data/ephemeral/home/Final_Project/Graph_matching/code/images/test'
    save_plot(ohlc, all_points_inverse, back_candles, save_dir, fname="inverse_head_and_shoulders", hs=False)