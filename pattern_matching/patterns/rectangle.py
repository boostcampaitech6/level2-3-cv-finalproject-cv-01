...
def save_plot(ohlc: pd.DataFrame, all_rectangle_points: List[int], backcandles: int, save_dir: str) -> None:
    """
...
def plot_rectangle_points(company, fig, ohlc, rectangle_points, back_candles):
    return fig


def plot_rectangle_pattern(data, company, base_fig, success_patterns): 
    # 2. rectangle pattern => 잘 안잡히는 패턴 같음
    ohlc_rectangle = get_ohlc(data)
    back_candles_rectangle = 20
    rectangle_points = find_rectangle_points(ohlc_rectangle, back_candles_rectangle)
    
    if rectangle_points:
        success_patterns.append("rectangle")
        rectangle_fig = go.Figure(base_fig)
        rectangle_fig = plot_rectangle_points(company, rectangle_fig, ohlc_rectangle, rectangle_points, back_candles_rectangle)
        st.plotly_chart(rectangle_fig, use_container_width=True)
...
if __name__ == "__main__":
    ...
    file = '/data/ephemeral/home/Final_Project/pattern_matching/data/SK Hynix_10y_1d_data.csv'
    # file = '/data/ephemeral/home/Final_Project/Graph_matching/code/data/eurusd-4h.csv'
    df   = pd.read_csv(file)
    ...
    # save_plot(ohlc, all_rectangle_points, backcandles, save_dir)
